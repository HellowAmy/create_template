...
def write_file(ctx, path):
    with open(path, "w") as file:
        file.write(ctx)
        print("[write : {}]".format(os.path.abspath(path)))
...
# 显示模版列表
def show_template_ls(path):
    print("< 模板列表 >")
    ls = get_template_ls(path)
    for i in range(len(ls)):
        basename = ls[i].split("/template/")
        s = "[{}] [{}]".format(i, basename[1])
        print(s)
...
if __name__ == "__main__":
    ...
    # 解析参数
    ls_template = get_template_ls(path_template)
    s_name = sys_name
